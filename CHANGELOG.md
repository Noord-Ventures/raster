...
- Homepage paints the 204 module grid on the specimen field. Component preview boxes and in-action scenes stay open so those verticals run next to the control. Button-group joins are a 1px `--divider` gap, both schemes.
- Inner-page 204 verticals paint on `html::before`. `--grid-line` is 0.04 paper / 0.05 dark — quieter than 0.08/0.10, still above the invisible 0.025. Homepage kills that overlay: boxed 204 cells only, no gutters through Raster / tagline / install. Chrome `--divider` stays so button-group seams do not vanish. Grotesque / Ag / caption on About and the starter specimen use unit leading and a 408 cell so they do not collide. Site chrome Appearance is Light / Dark / Auto (sliders mark, square selected cell). About Usage code blocks sit on the 204 column with the copy control inside the cell. Designer tiles that have a rights-clean still of the work show a poster crop; Piet Zwart, Paul Schuitema, and Otto Treumann stay type-only. Interfaces drops page-level 204 horizontals; Night's in-card map grid stays. Does not publish.
