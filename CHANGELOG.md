...
- Site 204 verticals paint on `html::before` so `overflow-x: clip` cannot drop `background-attachment: fixed`. Homepage and catalog specimens stay open to that layer. Does not publish.
