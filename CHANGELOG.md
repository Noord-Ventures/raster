...
- Publishable packages: `@raster/core`, `@raster/react`, `@raster/cli` at 0.3.0. Not private. Types ship in the tarball. `@raster/react` depends on `@raster/core` so CSS is not a second secret install.
...
- Documented CLI is `npx @raster/cli init`. `--registry` is implemented (HTTP(S) or a local directory; also `raster.json.registry`). `init --compat` still writes 0.1 class names.
