...
- Site 204 gutter overlay paints on `html::before` for Components and the rest. Homepage kills that overlay and keeps boxed 204 cells only — no gutter lines through Raster / tagline / install. `--divider` is the same ink as `--grid-line` (0.08 / 0.10). Button-group joins stay the 1px `--divider` gap. No 20px page frame.
