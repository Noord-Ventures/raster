...
- Site 204 module grid paints on `html::before` so `overflow-x: clip` cannot drop it. Home, `/components`, component detail, About, and Interfaces share that field. Type occupies a 204 cell. `--divider` is the same ink as `--grid-line`. Button-group joins stay the 1px `--divider` gap from the catalog recut. Quiet, readable on paper and dark. No 20px page frame.
