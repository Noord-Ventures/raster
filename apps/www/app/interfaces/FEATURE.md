...
| Wall | `/interfaces/wall` | Interfaces → Wall | Social feed. Feed → post → profile. Masonry on desktop, a text stream on the phone. First names from /work keepers only: Aziez, Jenny, Koen, Gianpiero. |
...
| Phone | ≤430 | Contents picker | Rail hides under 900. A stacked 44pt picker lists the six. Scene controls use the 44pt phone scale. Each boxed demo at 390 is a phone UI, not the desktop scene scaled down. Evening kitchens are text rows. Wall stories keep all four keepers as text. Night list first. |
