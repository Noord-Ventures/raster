...
| Wall | `/interfaces/wall` | Interfaces → Wall | Social feed. Feed → post → profile. Masonry on desktop, a stream on the phone. Comments sit in the inspect. First names from /work keepers only: Aziez, Jenny, Koen, Gianpiero. |
| Night | `/interfaces/night` | Interfaces → Night | Fleet management. List → unit → trip. A street you look down. Buildings, street, and lamps in frame. Not a facade wall crop. A route, one selected unit. |
