...
function Drive() {
  ...
  return <div className="cx cx-drive">
    ...
    <section><p className="cx-kicker">Cabin</p><div className="cx-temp"><button onClick={() => setTemp(temp - 1)}>−</button><strong>{temp}°</strong><button onClick={() => setTemp(temp + 1)}>+</button></div><div className="cx-quick"><button><Icon name="phone" size={16}/>Phone</button><button><Icon name="wifi" size={16}/>Online</button><button><Icon name="power" size={16}/>Charge</button><button><Icon name="sun" size={16}/>Climate</button></div><p className="cx-device"><span>Connected</span><b>Mara’s phone</b></p></section>
  </div>;
}
