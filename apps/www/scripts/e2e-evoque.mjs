// Network integration checks for the actual licensed Evoque embed.
// Build and serve the site first, then:
// SITE_URL=http://localhost:3100 PLAYWRIGHT_EXECUTABLE_PATH=/path/to/chrome node apps/www/scripts/e2e-evoque.mjs
...
const artifacts = await mkdtemp(join(tmpdir(), "vlak-evoque-"));
...
async function inspect(page, method) {
  return page.evaluate((name) => new Promise((resolve, reject) => {
    ...
    window.evoqueQaApi[name]((error, value) => {
...
try {
  ...
  await ready(page);
  assert.match(await page.locator(".cx-evoque-frame").getAttribute("src"), /034600db0cc94d64a7f3ccb19c7799fa/);
  ...
  const layout = await page.evaluate(() => {
    const frame = document.querySelector(".cx-evoque-frame").getBoundingClientRect();
    const credit = document.querySelector(".cx-evoque-credit").getBoundingClientRect();
  ...
  assert.match(await unavailable.locator(".cx-evoque-status").innerText(), /internet connection and WebGL/);
  ...
  await context.close();
  console.log("Evoque integration passed: real camera, paint, wireframe, responsive fit, reduced motion, offline fallback and retry");
