...
const HELP = `@raster/cli ${VERSION}, the monochrome design system

Usage
  npx @raster/cli init [--css-dir <dir>] [--components-dir <dir>] [--compat] [--registry <url>] [--overwrite]
  npx @raster/cli add <component...> [--overwrite] [--registry <url>]
  npx @raster/cli list
...
Next steps
  ...
  4. Add components:  npx @raster/cli add button dialog
`);
      break;
    }
    ...
    case "add": {
      if (positional.length === 0) {
        console.error("Nothing to add. Usage: npx @raster/cli add <component...>   (see: npx @raster/cli list)");
        process.exit(1);
      }
      ...
      for (const name of unknown) console.error(`✗ unknown component "${name}". See: npx @raster/cli list`);
