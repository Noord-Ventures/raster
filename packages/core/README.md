...
`vlak.css` is 84 KB (14 KB gzipped) and paints every component through `rs-*` classes. It is generated from the React package's StyleX leaves, so the two paths never drift. Individual files are exported too: `@noorddev/vlak/css/tokens.css`, `@noorddev/vlak/css/components/button.css`, and so on.
...
The same tokens as JSON: `@noorddev/vlak/tokens`. As custom properties: `css/tokens.css`.

## Registry

```ts
import { catalogComponents, vlakComponents } from "@noorddev/vlak";
```
